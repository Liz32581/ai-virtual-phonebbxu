...
export function loadMomentPostsForActiveUser(): MomentPost[] {
    const uid = getActiveUserIdentityId();
    return loadMomentPosts().filter(p => !p.ownerIdentityId || p.ownerIdentityId === uid);
}
...
export function getAllPostsForActiveUser(): MomentPost[] {
    return loadMomentPostsForActiveUser();
}
