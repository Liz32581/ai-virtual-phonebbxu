...
import { getActiveUserIdentityId } from "./active-user";
...
export function loadChatContactsForActiveUser(): ChatContact[] {
    const uid = getActiveUserIdentityId();
    return loadChatContacts().filter(c => !c.ownerIdentityId || c.ownerIdentityId === uid);
}
...
export function addChatContact(characterId: string): ChatContact | null {
    ...
    const newContact: ChatContact = {
        ...
        ownerIdentityId: getActiveUserIdentityId(),
    };
...
export function loadChatSessionsForActiveUser(): ChatSession[] {
    const uid = getActiveUserIdentityId();
    return loadChatSessions().filter(s => !s.ownerIdentityId || s.ownerIdentityId === uid);
}
...
export function createOrGetSession(contactId: string): ChatSession {
    const sessions = loadChatSessions();
    const uid = getActiveUserIdentityId();
    const existing = sessions.find(s => s.contactId === contactId && (!s.ownerIdentityId || s.ownerIdentityId === uid));
