...
import { getAllPosts, getAllPostsForActiveUser, deleteMomentPost, getUnreadMomentsNotifications, saveMomentsLastSeen, addMomentComment } from "@/lib/moments-storage";
...
export function MomentsFeed({ onCloseApp }: MomentsFeedProps) {
    ...
    const refreshPosts = useCallback(() => {
        ...
        setPosts(getAllPostsForActiveUser().filter(p => p.authorType === "user" || contactIds.has(p.authorId)));
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            window.removeEventListener(ACTIVE_USER_CHANGED_EVENT, handler);
        };
    }, [refreshPosts]);
