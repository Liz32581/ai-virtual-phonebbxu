...
import { loadChatSessions, loadChatContacts, loadChatSessionsForActiveUser, loadChatContactsForActiveUser, ChatSession, createOrGetSession, createGroupSession, pushChatMessage, addChatContact, loadChatMessages, getLastVisibleSessionMessage, getChatMessagePreview } from "@/lib/chat-storage";
...
export function ChatMessageList({ onCloseApp, activeSession, onSelectSession, onSelectMascot }: ChatMessageListProps) {
    ...
    useEffect(() => {
        if (!activeSession) {
            setSessions(loadChatSessionsForActiveUser());
        }
    }, [activeSession]);

    useEffect(() => {
        const refreshSessions = () => setSessions(loadChatSessionsForActiveUser());
        ...
        window.addEventListener(ACTIVE_USER_CHANGED_EVENT, refreshSessions);
        return () => {
            ...
            window.removeEventListener(ACTIVE_USER_CHANGED_EVENT, refreshSessions);
        };
    }, []);
        ...
        <div className="relative flex-1 h-full">
            ...
            >
                ...
                <div className="px-5 pt-2 flex flex-col">
                    {(() => {
                            const contactIds = new Set(loadChatContactsForActiveUser().map(c => c.characterId));
...
function ContactPicker({ onClose, onSelect }: { onClose: () => void; onSelect: (charId: string) => void }) {
    const contacts = loadChatContactsForActiveUser();
    const chars = loadCharacters();
