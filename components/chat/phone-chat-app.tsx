...
import { ChatSession, loadChatSessions, loadChatSessionsForActiveUser, pushChatMessage, hydrateChatStorage } from "@/lib/chat-storage";
...
export const PhoneChatApp = memo(function PhoneChatApp({ onClose, initialSessionId, onSessionChange, sharePayload, onShareDone }: PhoneChatAppProps) {
    ...
    useEffect(() => {
        hydrateChatStorage().then(() => {
            ...
            if (initialSessionId) {
                const s = loadChatSessionsForActiveUser().find(s => s.id === initialSessionId);
    ...
    useEffect(() => {
        const handler = (e: Event) => {
            ...
            if (!sessionId) return;
            const session = loadChatSessionsForActiveUser().find(s => s.id === sessionId);
            if (!session) return;
            setActiveMascot(false);
    ...
    // Reset chat app state when active user identity switches
    useEffect(() => {
        const handler = () => {
            setActiveSession(null);
            setActiveMascot(false);
